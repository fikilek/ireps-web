...
const fixture=()=>({parent:{id:tbId,execution:{status:"IN_PROGRESS"}},row:{id:"ROW1",tbId,salesAllMeterId:"00123",execution:{status:"IN_PROGRESS"},refs:{erfId:"ERF1",premiseId:null}},sales:{targetedBatchId:tbId,tbRefs:[{id:tbId,date:stamp,rowId:"ROW1",fieldWork:{status:"IN_PROGRESS",updatedAt:stamp,noAccess:[{date:"2026-09-13",time:"10:00:00",user:"FWR"}]}}]},history:{eventType:"BATCHED",tbId,salesId:"00123"},fence:{linkState:"LINKED",proposedTbId:tbId},visits:[{targetedBatchContext:{tbId,rowId:"ROW1",salesDocId:"00123",erfId:"ERF1"},accessData:{access:{hasAccess:"no",reason:"Locked gate"}},location:{gps:{lat:-28.5,lng:30.5}},capturedAt:stamp,media:[{tag:"noAccessPhoto",url:"gs://mock/photo.jpg"}]}],expectedVisits:1});
test("canonical readonly verification needs actual photo, numeric GPS, reason and exact variable counts",()=>{
 ...
 for(const mutate of [f=>f.visits[0].location.gps.lat=null,f=>f.visits[0].media[0].url="",f=>f.visits[0].accessData.access.reason="",f=>f.expectedVisits=2,f=>f.history=null,f=>f.fence.linkState="UNLINKED",f=>f.row.refs.premiseId="MISSING"]){const value=fixture();mutate(value);assert.equal(assessNoAccessEvidence(value).passed,false);}
