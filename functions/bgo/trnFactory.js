...
export function buildBgoBatchDoc({
  ...
  return {
    ...
    // DATA CONTRACT:
    // batchReleaseSummary is BGO batch-control truth.
    // It describes the handoff/release state of the child TRNs.
    // It is NOT live field execution truth.
    //
    // Live execution truth lives in trns.
    // A backend-maintained derivedExecutionSummary may mirror execution counts
    // only when it is recalculated from child TRNs.
    batchReleaseSummary: {
      totalRows: rowCount,
    ...
    // DATA CONTRACT:
    // derivedExecutionSummary is a derived mirror from child TRNs.
    // Source truth remains trns/{trnId}.
    // This object must only be updated by BGO execution-summary logic.
    derivedExecutionSummary: {
      sourceCollection: "trns",
      sourceField: "bgo.batchId",
      derivedAt: null,
      totalChildTrns: trnIds.length,
      totalNotExecuted: trnIds.length,
      totalAccepted: 0,
      totalInProgress: 0,
      totalCompleted: 0,
      totalSuccess: 0,
