...
export function assessNoAccessEvidence({ parent, row, sales, visits, history, fence, expectedVisits, baseline, premise = null, historical = false }) {
  ...
  const checks = {
    ...
    historyRetained: historical || history?.eventType === "BATCHED" && history.tbId === parent.id && history.salesId === row.salesAllMeterId, fenceConsumed: historical || fence?.linkState === "LINKED" && fence.proposedTbId === parent.id,
