...
import {SCHEMA_VERSION, PROJECT_ID, LM_PCODE, classifyScope, exactUpdateTime, updateTimeEqual, stable, sha256, collectStrings, parseStorageReference} from "./endumeniReset.helpers.js";
...
const REPORT_ROOT = args["output-dir"] ? path.resolve(args["output-dir"]) : null;
const mode = process.argv[2];
const serviceAccount = args["service-account"];

function assertCli() {
  if (!process.argv.includes("--historical-dev-inventory") || process.argv.includes("apply") || process.argv.includes("--apply")) throw new Error("TARGETED_BATCH_MAINTENANCE_RETIRED: only labelled historical DEV inventory or verification is available");
  if (!['inventory', 'verify'].includes(mode)) throw new Error("Usage: endumeni-reset.js <inventory|verify> --service-account <path> [--manifest <inventory.json>] [--confirm token]");
  if (args["project-id"] !== PROJECT_ID) throw new Error("WRONG_PROJECT");
  if (!REPORT_ROOT || !fs.existsSync(REPORT_ROOT) || !fs.statSync(REPORT_ROOT).isDirectory()) throw new Error("Explicit existing --output-dir required; no folders are created");
  if (!serviceAccount || !fs.existsSync(serviceAccount)) throw new Error("SERVICE_ACCOUNT_NOT_FOUND");
}
function writeNew(file, value) { if (!fs.statSync(path.dirname(file)).isDirectory()) throw new Error("OUTPUT_DIRECTORY_REQUIRED"); fs.writeFileSync(file, `${JSON.stringify(stable(value), null, 2)}\n`, {flag: "wx"}); }
function read(file) { return JSON.parse(fs.readFileSync(file, "utf8")); }
function timestamp() { return new Date().toISOString().replace(/[-:.]/g, ""); }
function manifestPath() { const file = args.manifest; if (!file) throw new Error("--manifest is required"); const resolved = path.resolve(file); const relative = path.relative(REPORT_ROOT, resolved); if (relative.startsWith("..") || path.isAbsolute(relative) || !path.basename(resolved).endsWith("-inventory.json")) throw new Error("MANIFEST_OUTSIDE_APPROVED_REPORT_ROOT"); return resolved; }
...
async function inventory(db, bucket) {
  ...
  const runDir = REPORT_ROOT, prefix = `historical-dev-${timestamp()}`; const targetFile = path.join(runDir, `${prefix}-targets.json`); const storageFile = path.join(runDir, `${prefix}-storage.json`);
  writeNew(targetFile, targets); writeNew(storageFile, storage);
  const result = {schemaVersion: SCHEMA_VERSION, projectId: PROJECT_ID, lmPcode: LM_PCODE, mode: "HISTORICAL_DEV_READ_ONLY", executableApproval: false, canonicalCreationApproval: false, createdAt: new Date().toISOString(), status: blockers.length ? "BLOCKED" : "PASSED", blockers: [...new Set(blockers)].sort(), files: {targets: {path: targetFile, sha256: sha256(fs.readFileSync(targetFile))}, storage: {path: storageFile, sha256: sha256(fs.readFileSync(storageFile))}}, counts: Object.fromEntries(Object.entries(targets).map(([name, rows]) => [name, rows.length])), storageCount: storage.length};
  const file = path.join(runDir, `${prefix}-inventory.json`); writeNew(file, result); console.log(JSON.stringify({status: result.status, manifest: file, counts: result.counts, blockers: result.blockers}, null, 2)); if (blockers.length) process.exitCode = 2;
}
...
async function verify(db, bucket, loaded, final = false) { if (!final) { await assertFresh(db, loaded.targets); await assertStorageFresh(bucket, loaded.storage); return {status: "PASSED", phase: "HISTORICAL_SNAPSHOT_COMPARISON", executableApproval: false, canonicalCreationApproval: false}; } const failures = []; for (const name of DELETE_COLLECTIONS) { const rows = await pageCollection(db, name); if (targetRows(rows).length) failures.push(`TARGETS_REMAIN:${name}`); } for (const row of loaded.storage) { const current = await storageState(bucket, row); if (current.state !== "MISSING") failures.push(`STORAGE_REMAINS:${row.objectPath}`); } return {status: failures.length ? "FAILED" : "PASSED", phase: "HISTORICAL_FINAL_COMPARISON", executableApproval: false, canonicalCreationApproval: false, failures}; }


async function apply() { throw new Error("TARGETED_BATCH_MAINTENANCE_RETIRED: use governed batch removal; direct mutation is prohibited"); }
