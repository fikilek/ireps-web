...
import { getGmrSalesCategory } from "../../../functions/reports/generalMonthlyReport.js";
...
test("prepared GMR backend rejects missing/invalid exact categories and never uses scalar roots", () => {
  ...
  for (const value of [null, { ...entry, extra: 1 }, { ...entry, riskScore: "0" }, { ...entry, riskTier: "" }]) {
    const sales = { leakageCategory: "LEGACY", monthlyCategories: { "2026-07": entry, "2026-08": value } };
    assert.equal(getGmrSalesCategory(sales, "2026-08"), null);
  }
});
