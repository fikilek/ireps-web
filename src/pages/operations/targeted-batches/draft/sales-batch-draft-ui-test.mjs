...
test("Clear draft cancellation preserves the draft; confirmation returns each Sales origin to its table", async () => {
 for(const source of ["PREPAID_SALES_NON_GPS","PREPAID_SALES"]) {
  ...
  const page=await load("../../TargetedBatchDraftPage.jsx",{
   ...
   "../../redux/geofencesApi":{useCreateGeoFenceMutation:mutation},
   ...
   "../../redux/salesTargetedBatchApi":{useGetSalesBatchDraftSnapshotQuery:()=>({data:{ready:false}}),useResolveSalesTargetedBatchMutation:mutation,useAssessSalesTargetedBatchMutation:mutation,useCreateSalesTargetedBatchMutation:mutation},
