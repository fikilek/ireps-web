...
test("one retained draft permits 1/30, rejects empty/31/duplicates; no automatic removal",()=>{
 ...
 const projected=projectSalesDraft(draft,live);assert.equal(projected.rows.length,2);assert.equal(projected.readyIds.length,0);
...
test("mixed retained Wards block Save/Create even when one meter is ineligible",()=>{
 ...
 const result=projectSalesDraft(draft,live,{geometry:f.ward.geometry});assert.equal(result.wards.length,2);assert.equal(Boolean(result.canSave),false);assert.equal(Boolean(result.canCreate),false);assert.match(result.gate,/multiple Wards/);
});
test("saved allowlist cannot grow; live failure/consumed fence block action; time passing never does",()=>{
 ...
 let result=projectSalesDraft(draft,live);assert.deepEqual(result.readyIds,[ids[0]]);assert.equal(Boolean(result.canCreate),true);assert.match(result.rows[1].reason,/saved population/);
 const realNow=Date.now;Date.now=()=>realNow()+365*24*60*60*1000;
 try{assert.deepEqual(projectSalesDraft(draft,live).readyIds,[ids[0]]);}finally{Date.now=realNow;}
 assert.equal(Boolean(projectSalesDraft(draft,{...live,ready:false}).canCreate),false);
 assert.equal(Boolean(projectSalesDraft(draft,{...live,fence:{...live.fence,targetedBatch:{...live.fence.targetedBatch,linkState:"LINKED"}}}).canCreate),false);
 assert.equal(Boolean(projectSalesDraft(draft,{...live,parent:{id:draft.id}}).canCreate),false);
});
...
test("undeployed, offline and blocked resolver calls have plain matching row states without changing Sales or the draft", () => {
 ...
 for(const error of [
  ...
  const result=projectSalesDraft(draft,live,{resolutionsCurrent:false,resolutionFailure:failure,geometry:f.ward.geometry});
...
test("waiting, pending, unavailable and successful recheck remain distinct even after a prior success", () => {
 ...
 assert.match(projectSalesDraft(draft,live,{resolutionsCurrent:false}).rows[0].reason,/Not located yet/);
 const pending=projectSalesDraft(draft,live,{resolving:true,resolutionFailure:failure});
 ...
 const failed=projectSalesDraft(draft,live,{resolutionFailure:failure});
 assert.equal(failed.rows[0].code,"RESOLUTION_SERVICE_UNAVAILABLE"); assert.deepEqual(failed.readyIds,[]);
 const recovered=projectSalesDraft(draft,live);
...
test("service errors retain genuine policy reasons and completed lookup outcomes", () => {
 ...
 const policyReason=projectSalesDraft(draft,live).rows[0].reason;
 const failure=salesDraftResolutionFailure({code:"functions/internal"},draft.source.type);
 assert.equal(projectSalesDraft(draft,live,{resolutionFailure:failure}).rows[0].reason,policyReason);
 draft.resolutions[ids[1]]={ready:false,code:"NO_EXACT_POSITION",reason:"Needs manual ERFing — NO_EXACT_POSITION"};
 assert.equal(projectSalesDraft(draft,live).rows[1].reason,"Needs manual ERFing — NO_EXACT_POSITION");
...
test("TB Draft translates provider and policy messages without changing internal codes or source records", () => {
 ...
 for(const message of ["Geocoding is unavailable; no failed-lookup flag was written", "Resolution is incomplete", "Sales data changed; resolution must be reassessed", "Coordinates and ERF must be resolved", "Verified geocoding evidence is required", "Targeted Batch reference integrity is unresolved", "Recheck resolution", "Geocoded position"]) {
  ...
  const row=projectSalesDraft(draft,live).rows[0];
 ...
 // Rules 18.4 (1.3.6): a located meter has no time limit, with or without an old expiresAt.
 const fresh=fixture();for(const row of Object.values(fresh.draft.resolutions))delete row.expiresAt;
 assert.equal(projectSalesDraft(fresh.draft,fresh.live).rows[0].ready,true);
 assert.equal(projectSalesDraft(fixture().draft,fixture().live).rows[0].ready,true);
