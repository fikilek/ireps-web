...
export default function TargetedBatchDraftSummary({ draft, model }) {
  return <div><div style={styles.summaryGrid}>{[["Retained meters",model.rows.length],["Ready meters",model.readyIds.length],["Left out",model.rows.length-model.readyIds.length],["Ward",model.wards.join(", ") || "Unresolved"]].map(([label,value]) => <div key={label} style={styles.summaryCard}><span>{label}</span><strong style={styles.summaryValue}>{value}</strong></div>)}</div>
