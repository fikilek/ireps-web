import { salesDraftWardLabel } from "./sales-batch-draft-model";
...
export default function TargetedBatchDraftSummary({ draft, model }) {
  return <div><div style={styles.summaryGrid}>{[["Retained meters",model.rows.length],["Ready meters",model.readyIds.length],["Left out",model.rows.length-model.readyIds.length],["Ward",model.wards.map(pcode => salesDraftWardLabel(pcode)).join(", ") || "Not found yet"]].map(([label,value]) => <div key={label} style={styles.summaryCard}><span>{label}</span><strong style={styles.summaryValue}>{value}</strong></div>)}</div>
