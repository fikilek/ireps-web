...
function resolveLocalSalesTargetScope(row = {}, expectedLmPcode = "") {
  ...
  const erfNos = uniqueNonBlank([
    String(row?.erfNo || "").trim(),
    ...matchingCandidates.map((candidate) => candidate.erfNo),
  ]);
  ...
  if (erfNos.length !== 1) {
    return {
      ...
      code:
        erfNos.length === 0
          ? "SALES_ERF_NUMBER_MISSING"
          : "SALES_ERF_NUMBER_CONFLICT",
      message:
        erfNos.length === 0
          ? `Meter ${salesAllMeterId || row?.meterNo || "NAv"} has no resolved ERF number.`
          : `Meter ${salesAllMeterId || row?.meterNo || "NAv"} has conflicting ERF numbers.`,
    };
  }
