...
export default function TargetedBatchDraftReview({ draft, model, live, drawing, geometry, busy, onRemove, onSave, onCreate, onClear, onResolve, feedback }) {
  ...
  return <section style={styles.panel}>
    <header style={styles.header}><div><p style={styles.eyebrow}>Sales Planning</p><h1 style={styles.title}>TB Draft</h1><p>{draft.id}</p></div>
      <div style={styles.headerActions}><button type="button" onClick={onResolve} disabled={busy}>Recheck resolution</button><button type="button" onClick={onClear} disabled={busy || Boolean(draft.uncertainRequest)}>Clear draft</button>
        <button type="button" style={styles.primaryButton} onClick={onCreate} disabled={busy || !model.canCreate}>Create</button></div></header>
    <TargetedBatchDraftSummary draft={draft} model={model}/>
    {feedback && <p role="alert" style={styles.integrityPanel}>{feedback}</p>}
    {!live?.ready && <p role="status" style={styles.sourceNotice}>{live?.error || "Loading current Sales, ERF, Ward and geofence data…"}</p>}
    <div style={styles.twoPane}><TargetedBatchDraftTable rows={model.rows} disabled={busy || Boolean(draft.uncertainRequest)} onRemove={onRemove}/>
      <div style={styles.pane}><SalesBatchDrawingControls drawing={drawing} saved={Boolean(draft.savedFence)} disabled={busy || !live?.ready || model.wards.length !== 1} canSave={model.canSave} onSave={onSave}/>
        ...
        <SalesTargetedBatchMap erfs={layers.erfs ? mapData.erfs : []} premises={layers.premises ? mapData.premises : []} meters={layers.meters ? mapData.meters.filter(showStatus) : []} hasDraftBoundary={Boolean(ward)} height={560}>
