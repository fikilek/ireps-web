...
// Rules 18.7: a nearby layer (ERFs, Sales, Premises or Assets) for a Ward and an area. TB Draft keeps
// an area 10 minutes (its area is fixed per draft); the GPS Sales map follows the screen (TB-R055.7).
function nearbyLayerEndpoint(keepUnusedDataFor) {
  return {
    keepUnusedDataFor,
...
export const salesTargetedBatchApi = createApi({
  ...
  endpoints: (rtkBuilder) => {
    ...
    // Targeted Batch rules TB-R057 (1.3.55): Batch Stats, counted live by the server and never kept.
    // keepUnusedDataFor covers the count itself: reopening the page while it runs joins the running count instead of starting a second one.
    getBatchStatsByLm: rtkBuilder.query({ ...callSalesBatch("getBatchStatsCallable"), keepUnusedDataFor: 300 }),
